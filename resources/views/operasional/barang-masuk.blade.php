    ...
    <script>
        ...
        function carialat() {
            ...
            $.get('/operasional/item-kode/' + encodeURIComponent(kode), function(item) {
                ...
                $("#sudahmaxreuse").val('0');
                $("#punyariwayatkeluar").val(parseInt(item.jumlah_keluar) > 0 ? '1' : '0');
                $("#infoalat").html(`<b>${item.kode_unik}</b> - ${item.nama_bmhp}<br>Reuse sekarang: ${item.reuse_ke}/${item.max_reuse}<br>Status: ${item.status}<br>Unit terakhir: ${item.last_unit ?? '-'}`);
        ...
        function nilai() {
            ...
            return { _token: "{{ csrf_token() }}", cssd_item_id, tanggal_masuk, tanggal_penggunaan, nama_section_pengguna, no_rm, nama_dpjp, nama_perawat, kondisi_awal, metode_steril, petugas_penerima_pencucian, petugas_pengemasan, petugas_sterilisasi };
        }
