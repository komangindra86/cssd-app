...
@section('title', 'Dispose / Expired')

@section('content')
    <div class="p-6">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-slate-800">Dispose / Expired</h1>
            <p class="mt-1 text-sm text-slate-500">Daftar item alat yang tidak boleh dipakai ulang.</p>
        </div>
        <x-item-status-table table-id="tbdispose" />
    </div>
@endsection
...
@push('scripts')
    <script>
        ...
        function datatableitem() {
            tabeldispose = $("#tbdispose").DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: '/operasional/item-data',
                    data: function(d) {
                        d.status = 'DISPOSE_EXPIRED';
                    }
                },
                pageLength: 10,
